...
void GroupMgr::LoadGroups()
{
    ...
    TC_LOG_INFO("server.loading", "Loading Group instance saves...");
    {
        ...
        QueryResult result = CharacterDatabase.Query("SELECT gi.guid, i.map, gi.instance, gi.permanent, i.difficulty, i.resettime, COUNT(g.guid) "
            ...
            "LEFT JOIN character_instance ci LEFT JOIN `groups` g ON g.leaderGuid = ci.guid "
            "ON ci.instance = gi.instance AND ci.permanent = 1 "
            "GROUP BY gi.guid, i.map, gi.instance, gi.permanent, i.difficulty, i.resettime "
            "ORDER BY gi.guid");
        if (!result)
        {
